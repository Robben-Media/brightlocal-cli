...
func (cmd *AuthStatusCmd) Run(ctx context.Context) error {
	...
	envKey := os.Getenv("BRIGHTLOCAL_API_KEY")
	envOverride := envKey != ""
	...
	switch {
	case envOverride:
		fmt.Fprintln(os.Stderr, "Status: Using BRIGHTLOCAL_API_KEY environment variable")
	case hasKey:
	...
	default:
		...
		fmt.Fprintln(os.Stderr, "Run: brightlocal-cli auth set-key --stdin")
	}
