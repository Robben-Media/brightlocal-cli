...
import (
	...
	"github.com/builtbyrobben/brightlocal-cli/internal/config"
)
...
const (
	apiKeyKey          = "api_key"
	keyringPasswordEnv = "BRIGHTLOCAL_CLI_KEYRING_PASS"
	keyringBackendEnv  = "BRIGHTLOCAL_CLI_KEYRING_BACKEND"
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set BRIGHTLOCAL_CLI_KEYRING_BACKEND=file and BRIGHTLOCAL_CLI_KEYRING_PASS=<password> to use encrypted file storage instead",
